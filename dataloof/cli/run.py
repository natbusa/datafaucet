...
from .application import DataloofApp
...
class DlfRunApp(DataloofApp):
    name = Unicode(u'dataloof-run')
    description = "Executing a dataloof notebook"
    ...
    def run_single_notebook(self, filename):
        ...
        init_str = dedent(f"""
            # added by dlf-run
            import dataloof
            dataloof.files.set_current_filename('{fullpath_filename}')
            dataloof.project.load(profile='{self.profile}', rootdir_path='{self.rootdir}')
            """)
