...
def dataloof_parser():
    ...
    group.add_argument('--version', action='store_true',
                       help="show the dataloof command's version and exit")
...
def list_subcommands():
    """List all dataloof subcommands

    searches PATH for `dataloof-name`

    Returns a list of dataloof's subcommand names, without the `dataloof-` prefix.
    Nested children (e.g. dataloof-sub-subsub) are not included.
    """
    subcommand_tuples = set()
    # construct a set of `('foo', 'bar') from `dataloof-foo-bar`
    ...
    subcommands = set()
    # Only include `dataloof-foo-bar` if `dataloof-foo` is not already present
...
def _path_with_self():
    """Put `dataloof`'s dir at the front of PATH

    Ensures that /path/to/dataloof subcommand
    will do /path/to/dataloof-subcommand
    even if /other/dataloof-subcommand is ahead of it on PATH
    """
    scripts = [sys.argv[0]]
    if os.path.islink(scripts[0]):
        # include realpath, if `dataloof` is a symlink
    ...
    for script in scripts:
        ...
        if os.path.isdir(bindir) and os.access(script, os.X_OK):   # only if it's a script
            ...
            # avoids missing subcommands when dataloof is run via absolute path
